...
const buildChart = (): void => {
  ...
  chartInstance = ChartUtils.buildUserGrowthLine(canvasRef.value, props.labels, props.counts);
};
...
<template>
  <div class="rounded-2xl border border-slate-200 bg-white p-6 space-y-4">
    <header>
      <h3 class="text-base font-semibold text-[#0B2C3D]">User Growth</h3>
      <p class="text-xs text-slate-500">Total registered users over time</p>
    </header>
